window.onload = function() {
    ...
    //before the (?) it is path parameter and after ? it is a query parameter
    fetch(`https://api.unsplash.com/photos/${search_params.get('id')}?client_id=${API_key}`).then(convert_to_json)// Convert response to JSON
    .then(function(data) {                                                                                                  
        loadDetail(data);
